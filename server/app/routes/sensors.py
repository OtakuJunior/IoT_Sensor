...
def update_sensor(updated_sensor : SensorUpdate, sensor_id : str, db : Session = Depends(get_db), admin: dict = Depends(require_admin)):
...
def delete_sensor(sensor_id: str, db:Session = Depends(get_db), admin: dict = Depends(require_master)):
