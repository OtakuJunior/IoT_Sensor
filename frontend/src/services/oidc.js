...
// Timeout for the scheduled token refresh
...
export function hasRole(role) {
  ...
  if (
    ...
    for (const r of payload.resource_access[clientId].roles)
      roles.add(String(r));
  }
...
export async function handleCallback(currentUrl) {
  ...
  scheduleRefresh();
  try {
    sessionStorage.removeItem("oidc.tmp");
  } catch (e) {
    console.error(e);
  }
...
export function logout() {
  ...
  const endSession =
    ISSUER && ISSUER.replace(/\/$/, "") + "/protocol/openid-connect/logout";
